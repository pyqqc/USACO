...
#    print("------------cmblineykres",cmblineykres)
    ...
    #遍历comblineykres里的每个元素，首先看每个元素里有几个线段L，取掉线段涉及的点以后，得到余下线段组合
    NL=int(N/2)
    fcombress=[]
    for comblineyk in cmblineykres:
#        print("-------------select a y loop comblineyk",comblineyk)
        L=len(comblineyk)
        NLL=NL-L
#       print("L,NLL",L,NLL)
        rmdots=removeylines(comblineyk,poss)
        NNC=int(len(rmdots)/2)
        liners=combdotsK(rmdots,NNC)
#        print(" remine lines combine select N noteconected",liners,NNC)
        combrsl=liners
#        print("go a combrsl",combrsl)
        comblineykl=list(map(list,comblineyk))
#        print("comblineykl",comblineykl)
        if combrsl!=[]:
            for combrl in combrsl:
                fcombres=comblineykl+list(combrl)
                fcombress.append(fcombres)
        else:
            fcombress.append(comblineykl)
    print("fcombress,len",fcombress,len(fcombress))
    
    #去重
    print("------removing same lines")
    fcmbresults=[]
    for fmb in fcombress:
#            print("fcmbresults",fcmbresults)
#            print("fmb,len",fmb,len(fmb))
        if fcmbresults==[]:
            fcmbresults.append(fmb)
        else:
            have=0
            for lines in fcmbresults:
                if linessame(lines,fmb)==True:
#                        print("alreay have in result",fmb)
                    have+=1
                else:
                    pass
#                       print("lines fmb not the same",lines,fmb)
            if have==0:
                fcmbresults.append(fmb)
